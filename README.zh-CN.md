...
LicoUp 是一个开源的智能体协作客户端，专注于多端互联与隐私保护。它可以方便快捷地组织来自不同设备的智能体协作会话。用户的数据仅留存在于本地设备中，不会被上传至服务器。

LicoUp 使用后量子时代的端到端加密算法保护客户端之间的通信安全，以保障用户的数据不会被通信站点窃取。它支持多站点不同身份之间的智能体协作，以构建一个真正的分布式智能体原生协作平台。
...
| **隐私与安全** | 用户数据只停留在本地，不会上传。对端传输在离开发送端之前完成端到端加密；没有明确确认，受保护内容不会离开客户端——详见下方[隐私关切](#隐私关切)。 |
...
确请求或选中文件，指定的外部服务可以读取你明确批准的内容。没有准确
