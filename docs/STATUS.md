...
| Assistant workflow and Subagent MCP | implemented in source | Four closed Assistant tools expose Profile ranking plus execute/inspect/cancel for assistant-temporary workflows. The MCP-bound Agent must be the active designated Assistant Membership. Execute performs local preflight, freezes exact Membership bindings and a privacy-safe route receipt before effects, and returns dynamic failure once without implicit retry. Direct `lico_subagent_*` operations require an active manager Membership and remain separate, while the persistent Conversation host is the sole run, turn, and transcript owner. |
