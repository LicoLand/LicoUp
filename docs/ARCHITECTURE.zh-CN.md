...
| 智能体对话 | 新建会话、原生继续会话、实时事件投影，以及不支持中途接入时的逐轮降级 |
...
结论。当前[支持状态](releases/client-support-matrix.zh-CN.md)分别记录这些结论，绝不把
模拟器或源码检查提升成真机或发布证明。
...
LicoLite 协作是独立安装的插件。用户完成安装并主动启用前，默认启动和导航不会加载它。
...
已验证身份生效；身份不一致时关闭失败。本源码树不捆绑 LicoLite 服务端运行器：这些控制
