...
| Regression | Codec vectors, malformed-frame property tests, exact-session conversation parity, cancellation, and Secure Mesh ACP relay tests. |
...
The client creates and continues local native-agent conversations. An adapter
first uses an official protocol, SDK, or structured command surface. Exact native
continuation must preserve the native session identity, effective model and
...
The packaged adapter set is Antigravity, Claude Code, Codex, Cursor, GitHub Copilot,
Hermes Agent, Kilo Code, Kimi Code, OpenClaw, OpenCode, and Pi Agent. The current
reducer summary is `0 ready / 0 failed / 2 blocked / 9 unverified`. Only `ready`
adapters enable the normal composer; all other states remain visible and fail
closed without blocking unrelated client functions.

Regression: target-specific new/continue tests, both native-to-Arc and Arc-to-native
continuation, stream ordering, cancellation, restart/cleanup, safe rendering,
privacy projection, and release-UI parity reducer.
...
## Scenario S-06 — End-to-End Encryption and Mobile Relay
...
Production paths use the shared Rust Secure Client Mesh core. Android and iOS
...
Run the smallest affected module set first. Run the complete `npm run
client:verify` exactly once only after all changed modules and integration edges
have passed. Repeating the complete regression during development is prohibited
because it consumes resources shared with parallel work.
