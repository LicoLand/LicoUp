...
状态指令支持多行文本，包括换行、回车和制表符，仍受现有 16 KiB UTF-8 大小限制。
其他控制字符及首尾空白仍不允许。标签保持单行。
...
输入框上方胶囊行先放 Assistant 身份胶囊、再放 Adaptive Flywheel 胶囊：点击名称在
界面中央打开独立助手配置框，复用既有 Agent、模型和推理强度字段；胶囊尾部一枚紫色
小开关控制它是否参与后续发送。发送按钮前方的模型展示器显示激活助手已选的模型与
思考强度，点击打开同一配置框；Assistant 关闭时展示器消失。助手档案与工作流绑定独立
保存。关闭开关不取消已经运行的 turn；中断仍由显式取消按钮负责。
