...
参与者。输入框上方提供工作空间胶囊、显示当前主智能体的飞轮胶囊，以及圆形编辑
按钮。悬停飞轮胶囊可选择智能体及其模型（Lico Agent 使用 Gateway 供应商 A–Z；
...
打开**适应性飞轮**可选择主智能体并配置代码工程角色。Designer 由前后端共享；
Worker 和 Reviewer 分别提供独立的后端与前端分配，每个分配均可设置智能体、模型和
思考强度。保存后的分配会投影到本机下属智能体 MCP，但不会暴露可执行文件路径或
原始文件；唯一配置源是私有客户端状态中的 `adaptive-flywheel.toml`。所有角色
选择器共享同一次真实目标扫描：任一已检测且可运行的智能体均可用于任一角色；模型
优先来自该智能体的原生模型目录，其次来自已验证的本机配置或缓存。静态目录只补充
已知模型元数据，不会覆盖成功的原生目录响应。
