...
优先级。Designer 由前后端共享；Worker 和 Reviewer 分别提供独立的后端与前端分配，
每个分配均可设置智能体、模型和思考强度。保存后的分配会投影到本机下属智能体 MCP，
但不会暴露可执行文件路径或原始文件；唯一配置源是私有客户端状态中的
`adaptive-flywheel.toml`。所有角色选择器共享同一次真实目标扫描：任一已检测且可
运行的智能体均可用于任一角色；模型优先来自该智能体的原生模型目录，其次来自已验证
的本机配置或缓存。静态目录只补充已知模型元数据，不会覆盖成功的原生目录响应。
