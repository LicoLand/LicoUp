...
打开**适应性飞轮**可配置日常对话与代码工程角色。**日常对话**是配置中的优先级列表，
第一个胶囊为默认调度归属；列表顺序同时也是 Lico 群聊当前对话在额度、余额、限流或
供应商容量失败时的自动回退链。回退成功后会把**当前对话**切换到实际成功的胶囊，
但不会重排日常对话列表。群聊输入框飞轮中的**当前对话**选择方式与日常对话相同；
若与日常对话第一优先级不一致，则以当前对话的智能体为准。再次保存适应性飞轮会把
当前对话重新同步为日常对话的第一个胶囊。日常对话以圆形加号开始：点击后向右展开
为胶囊搜索框，并横向展开三张悬浮卡片（智能体、对应模型、思考强度与 Fast 开关）；
确认后保存为胶囊。仅点击胶囊上的关闭图标可移除；长按拖动可调整顺序，顺序即生效
优先级。**代码工程**中设计师、执行者、审查官使用与日常对话相同的多选胶囊
（不含 Fast）。设计师由前后端共享，顺序即优先级。执行者与审查官按列表投影
到前后端线：第一个胶囊为后端，第二个为前端，仅一个胶囊时前后端共用。保存后的分配
会投影到本机下属智能体 MCP，但不会暴露可执行文件路径或原始文件；唯一配置源是私有
客户端状态中的 `adaptive-flywheel.toml`。所有角色选择器共享同一次真实目标扫描：
任一已检测且可运行的智能体均可用于任一角色；模型优先来自该智能体的原生模型目录，
其次来自已验证的本机配置或缓存。静态目录只补充已知模型元数据，不会覆盖成功的原生
目录响应。
