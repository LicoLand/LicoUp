...
[English](USER-GUIDE.md) · 简体中文 · [首页](../README.zh-CN.md)

Lico Arc 仍处于早期 Alpha 阶段。在重要场景中使用某个平台或功能前，请先查看
[支持状态](releases/client-support-matrix.zh-CN.md)。
...
继续对话时，Lico Arc 优先调用智能体原生的接入或恢复能力。如果适配器不能在智能体
...
LicoLite 协作是独立插件，默认客户端不会加载或查询它。
...
Lico Arc 源码树不包含 LicoLite 服务端运行器。因此，客户端构建成功不能证明已经获得
服务端制品，也不能证明已经启动本地部署。

安装或启用插件不会授予持续的数据传输权限。如果 MCP 操作需要访问外部服务，bridge 会
先创建一份不发起传输的预览。请在 Lico Arc 中检查目标、用途、准确请求和每个选中文件，
...
1. 选择接收内容的 Lico Arc 客户端。
2. 检查具体消息或文件以及目标客户端。
3. 只批准这一次传输。
4. Lico Arc 在发送设备上加密内容。
5. 接收客户端校验并解密。

Lico Arc 不信任中转端处理明文，只向它发送密文和完成本次传输所需的最少路由信息。
更换目标或修改内容后，必须重新确认。
...
如需了解客户端内部结构，请阅读[架构说明](ARCHITECTURE.zh-CN.md)。
