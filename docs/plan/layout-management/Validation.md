...
| REQ-002 | Bundle contract requires shell, destination builders, styled components, tokens, preview, and declared surface/viewport variants without importing implementations. | Desktop/mobile isolated bundle tests and goldens prove complete, materially different component trees. |
...
| REQ-005 | Resolver tests distinguish `desktop/medium` from `mobile/medium`, use constraints/capabilities, and never persist a resize. | Desktop/mobile constraint matrices prove surface-correct variants and stable selected identity. |
...
| REQ-014 | Scoped Better Plan checks and requirement/file/command traceability prove foundation contracts. | Renderer child receipts plus integration build, launch, independent Android verification, and scoped family checks prove the product. |
...
| REQ-014 | All non-skipped local requirements in foundation, desktop, mobile, and integration | `layout-management/integration` final validation records the family receipt. |
...
- `test/layout/layout_integration_test.dart`, `layout_settings_test.dart`, and `layout_state_continuity_test.dart` — central composition, selector transaction, cold start, and mounted switch behavior.
...
The architecture node shall add `npm run client:verify:layouts` for `verify-layout-boundaries.mjs`; renderer children shall add profile-only test selectors, and the integration child shall add `npm run client:test:layouts`. These commands are planned evidence until their owning nodes create and self-test them.
