...
5. Only when a named Developer ID or App Store channel is requested, sign, notarize, staple, publish and download the same digest; verify that channel's trust and update/rollback continuity and record its result separately from GitHub Release readiness.
...
REQ-MAC-001 is proven by steps 1–2; REQ-MAC-002 by step 3; REQ-MAC-004 by step 4; REQ-MAC-003 is evaluated by step 5 only for a requested channel. Missing external authority keeps only that channel unready.
