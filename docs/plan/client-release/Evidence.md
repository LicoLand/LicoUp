...
| BLK-014 | REQ-PROD-002 | Closed — source + fresh verification | Dispatch uses durable per-target outbox identity, bounded retries, restart recovery, duplicate suppression, and bounded typed attachments; focused tests cover success, failure, restart, and limits. |
...
| BLK-018 | REQ-SEC-001 | Closed — source + fresh verification | Credential export is high-risk and requires local user confirmation; remote trust or encrypted transport alone cannot authorize export. |
...
| BLK-032 | REQ-PROD-004, REQ-SEC-001 | Closed — source + fresh verification | Provider credential deletion is account-scoped, native, authorization-bound, and confirmed by reread before account metadata is removed. |
