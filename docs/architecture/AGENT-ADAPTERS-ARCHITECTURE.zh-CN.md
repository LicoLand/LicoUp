...
5. **禁止强制回复格式**：原生 Agent 的对话回复按它原本产出的样子传递。LicoUp 不下发任何约束 Agent 可见文本的回复 schema、响应格式或输出契约；自然语言回复也不会因为缺少格式而被视为无效、空回复或弃权。宿主语义从 Agent 实际说了什么中得出，绝不反过来要求它必须怎么说。源码门禁强制该边界：在原生 Agent 回合上强制回复格式键会使 `client:verify:agent-native-output` 失败，上述各处边界声明也必须始终存在。
