...
| 智能体对话 | 单聊与群聊共用统一 Conversation 模型；Human 与 Agent Principal 通过显式 Membership 参与。新建与原生续接会话保留进程内、可唤醒的进度；活动轮次在支持时使用原生 steer，否则在准确会话的安全边界继续。原生会话是适配器拥有、私下绑定到 Membership 的执行细节。本地[下属智能体 MCP](../protocols/subagent-mcp.zh-CN.md)只按 `conversationId + membershipId` 调度，不暴露原生续接路径 |
