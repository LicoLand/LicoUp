...
| 智能体发现 | 只并发探测 Agent 扫描路径清单中的命名二进制、配置和历史目录。不遍历 PATH、个人资料库根、照片/音乐库或网络宗卷。家目录只从环境变量读取；macOS firmlink 等价路径按同一规则分类。未使用智能体的探测对其他 App 容器只做词法分类，不去 stat。Token 用量在打开监测页之前不会扫描 |
