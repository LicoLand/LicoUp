...
5. **No Imposed Reply Format**: A native Agent's conversational reply is relayed as the Agent produced it. LicoUp sends no reply schema, response format, or output contract that constrains the Agent's visible text, and a natural-language reply is never invalid, empty, or an abstention for lacking one. Host semantics are derived from what the Agent said, never required of what it must say. The source gate enforces this boundary: an imposed reply-format key on a native Agent turn fails `client:verify:agent-native-output`, and the statements of the boundary above must remain present.
