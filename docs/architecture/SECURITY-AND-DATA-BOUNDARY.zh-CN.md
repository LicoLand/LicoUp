...
在已接受的工作流重构中，IPC caller 准入、Conversation Membership、操作 grant 与
操作系统隔离分别成立。不可信插件需要与威胁模型相符的进程、权限和资源隔离；
已验证 caller 或独立工作目录本身不是沙箱。使用原平台和组件边界，不能为协作扩大权限。

撤权阻止新的控制和披露；已知 invocation 的认证结果仍可按原身份和受限可见范围
结算该调用。记录事实不续授权，不允许新效果，也不授权 successor 读取结果内容。
这些要求须在实际 adapter/store 验证；本节不声称已经提供新的 OS 沙箱。
