...
| Agent conversations | New and native continued sessions, live event projection, and a turn-by-turn fallback when mid-run input is unavailable |
...
| Usage statistics | Local token aggregation by agent or model with a 30-day default and selectable time window |
...
[support matrix](releases/client-support-matrix.md) records them without
...
LicoLite collaboration is a separately installed plugin. It is absent from
...
a LicoLite server runner: these controls establish a client deployment
