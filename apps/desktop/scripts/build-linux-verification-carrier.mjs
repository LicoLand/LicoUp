...
const verificationRoot = path.join(workspaceRoot, "build", "apps", "desktop", "verification", "linux-arm64");
...
function assertArm64Bundle() {
  if (process.platform !== "linux" || !["arm64", "aarch64"].includes(process.arch)) {
    throw new Error("Linux ARM64 verification carriers must be produced on a native Linux ARM64 runner.");
  }
  for (const fileName of ["licoup", "licoup-cli"]) {
...
function main() {
  ...
  const archivePath = path.join(verificationRoot, "LicoUp-linux-arm64-verification.tar.gz");
  ...
  const signingKeyPath = path.resolve(requiredEnvironment("LICO_LINUX_VERIFICATION_SIGNING_KEY_PATH"));
  const signingKeyId = requiredEnvironment("LICO_LINUX_VERIFICATION_SIGNING_KEY_ID");
  if (!existsSync(signingKeyPath)) {
    throw new Error("Linux ARM64 verification signing key file is missing.");
  }
  const privateKey = createPrivateKey(stableReadFile(signingKeyPath, {
    maxBytes: 64 * 1024,
  }));
  if (privateKey.asymmetricKeyType !== "ed25519") {
    throw new Error("Linux ARM64 verification signing key must be Ed25519.");
  }
  ...
  if (sha256(archivePath) !== digest) {
    throw new Error("Linux ARM64 verification carrier changed while it was signed.");
  }
  writeFileSync(
    path.join(verificationRoot, "verification-manifest.json"),
    `${JSON.stringify({
      schemaVersion: "licomesh.client-linux.verification-carrier.v1",
  ...
  console.log(`Linux ARM64 verification carrier ready: ${path.relative(workspaceRoot, archivePath)}`);
}
