...
export async function checkCrateCoreAndFacadeBounds(context) {
  ...
  const cliEntrypoint = await readText("crates/licoup-native/src/bin/licoup.rs");
  const cliPresentation = await readText(
    "crates/licoup-native/src/bin/licoup/presentation.rs"
  );
  const cliCommands = await readText("crates/licoup-native/src/ffi/commands/mod.rs");
  assert(
    /CliExecution::Usage\s*=>\s*print_usage\(\)/u.test(cliEntrypoint) &&
      /for line in licoup_native::ffi::commands::cli_command_help\(\)\s*\{\s*eprintln!\("\{line\}"\);\s*\}/u.test(cliPresentation),
    "CLI usage must render the help generated by native command admission"
  );
  // Functional route/option parity is exercised by cli_command_contract_cases;
  // this boundary check only follows the shared registry projections.
  assert(
    /pub fn cli_command_help\(\) -> Vec<String> \{\s*build_command_table\(\)\.help_text\(\)\s*\}/u.test(cliCommands) &&
      /pub fn cli_command_schemas\(\) -> Vec<CliCommandSchema> \{\s*build_command_table\(\)\.schemas\(\)\s*\}/u.test(cliCommands) &&
      /pub fn admit_cli_command\(args: Vec<String>\) -> Result<AdmittedCommand> \{\s*build_command_table\(\)\.admit\(args\)\s*\}/u.test(cliCommands),
    "CLI help, machine catalog, and argument admission must use one command registry"
  );
