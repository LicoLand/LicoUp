...
所有打包桌面适配器都把已接受轮次交给当前客户端数据根唯一的本机 CLI 宿主。关闭
LicoUp 只会移除 GUI 观察者，不会取消、steer、截断活动轮次或让它超时。替换后的客户端
或 stdio 代理使用进程内游标（GUI 完整重启后从零开始）重新附着同一个 Conversation 作用域
轮次句柄，按序重放已提交帧，再继续接收实时输出。宿主只使用客户端私有的本机 IPC；
canonical Conversation Event 是持久重放权威，每个活动轮次的 16 MiB 可丢弃缓存只保存热尾部，游标
低于缓存下界时回落到带索引的 Conversation 存储。最后一个客户端离开且全部已接受轮次
终止后，宿主退出。
