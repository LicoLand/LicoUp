...
Widget buildClassicMobileCompactShell(
  BuildContext context,
  LayoutShellBuildContext data,
) {
  _validateEnvironment(data, LayoutViewportClass.compact);
  return _ClassicMobileShell(data: data, compact: true);
}

Widget buildClassicMobileMediumShell(
  BuildContext context,
  LayoutShellBuildContext data,
) {
  _validateEnvironment(data, LayoutViewportClass.medium);
  return _ClassicMobileShell(data: data, compact: false);
}
...
void _validateEnvironment(
  ...
  if (data.environment.surface != LayoutRuntimeSurface.mobile ||
    ...
    throw const FormatException('classic_mobile_shell_contract_invalid');
  }
}

final class _ClassicMobileShell extends StatelessWidget {
  const _ClassicMobileShell({required this.data, required this.compact});
  ...
  Widget build(BuildContext context) {
    ...
    final media = (inheritedMedia ?? const MediaQueryData()).copyWith(
      textScaler: TextScaler.linear(
        ClassicMobileMetrics.boundedTextScale(environment),
      ),
      disableAnimations:
    ...
    return RestorationScope(
      restorationId:
          '$classicMobileRestorationPrefix.${environment.viewport.name}.shell',
      child: MediaQuery(
        data: media,
  ...
  Widget _buildConstrainedShell(
    ...
    final adaptiveHorizontalPadding = math.min(
      ClassicMobileMetrics.horizontalPadding(environment),
      math.max(classicMobileTokens.spacingUnit, constraintWidth * 0.06),
    );
    ...
    final boundedContentWidth = math.min(
      classicMobileTokens.contentMaxWidth,
      contentWidth,
    );
    final topPadding =
        safeInsets.top +
        (compact
            ? classicMobileTokens.spacingUnit
            : classicMobileTokens.spacingUnit * 1.5);
    final bottomClearance = ClassicMobileMetrics.composerClearance(environment);

    return ColoredBox(
      key: ValueKey<String>(
        'classic-mobile-${environment.viewport.name}-shell',
      ),
        ...
        child: Align(
          ...
          child: ConstrainedBox(
            ...
            child: AnimatedPadding(
              key: const ValueKey<String>('classic-mobile-composer-clearance'),
              duration: ClassicMobileMetrics.motionDuration(environment),
              curve: Curves.easeOutCubic,
              padding: EdgeInsets.only(bottom: bottomClearance),
              child: compact
                  ? _ClassicCompactComposition(data: data)
                  : _ClassicMediumComposition(
                      data: data,
...
final class _ClassicCompactComposition extends StatelessWidget {
  const _ClassicCompactComposition({required this.data});

  final LayoutShellBuildContext data;

  @override
  Widget build(BuildContext context) => Column(
    key: const ValueKey<String>('classic-mobile-compact-card-stack'),
    children: [
      _CompactContextualNavigation(data: data),
      const SizedBox(height: ClassicMobileMetrics.compactStackGap),
      Expanded(
        child: data.components.panel(
          context,
          key: const ValueKey<String>(
            'classic-mobile-compact-destination-panel',
          ),
          emphasized: true,
...
final class _CompactContextualNavigation extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final interactiveExtent = ClassicMobileMetrics.interactiveExtent(
      environment,
    );
    ...
    final titleStyle = baseTitleStyle?.copyWith(
      ...
      fontSize:
          (baseTitleStyle.fontSize ?? 16) * classicMobileTokens.typographyScale,
    ...
    return data.components.panel(
      context,
      key: const ValueKey<String>(
        'classic-mobile-compact-contextual-navigation',
      ),
      child: Semantics(
        container: true,
            ...
            children: [
                ...
                child: DecoratedBox(
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(
                      classicMobileTokens.cardRadius * 0.6,
                    ),
                  ),
                  child: Padding(
              ...
              Expanded(
                child: Text(
                  activeLabel,
                  key: const ValueKey<String>(
                    'classic-mobile-active-destination-label',
                  ),
                  maxLines: 2,
              ...
              Focus(
                ...
                child: PopupMenuButton<ClientSection>(
                  key: const ValueKey<String>(
                    'classic-mobile-compact-navigation-trigger',
                  ),
                  tooltip: activeLabel,
                        ...
                        child: Semantics(
                          ...
                          child: Row(
                            children: [
                              ...
                              SizedBox(
                                width: classicMobileTokens.spacingUnit * 1.5,
                              ),
                              Expanded(
                                child: Text(
...
final class _ClassicMediumComposition extends StatelessWidget {
  const _ClassicMediumComposition({
    required this.data,
  ...
  Widget build(BuildContext context) {
    ...
    final scaledNavigationWidth =
        ClassicMobileMetrics.mediumNavigationWidth +
        (ClassicMobileMetrics.boundedTextScale(environment) - 1) * 28;
    ...
    return Row(
      key: const ValueKey<String>('classic-mobile-medium-card-stack'),
      ...
      children: [
        ...
        const SizedBox(width: ClassicMobileMetrics.mediumStackGap),
        Expanded(
          child: data.components.panel(
            context,
            key: const ValueKey<String>(
              'classic-mobile-medium-destination-panel',
            ),
            emphasized: true,
            child: _DestinationFocusAnchor(data: data),
          ),
...
final class _MediumContextualNavigation extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return data.components.panel(
      context,
      key: const ValueKey<String>(
        'classic-mobile-medium-contextual-navigation',
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Semantics(
            ...
            child: Text(
              ...
              key: const ValueKey<String>(
                'classic-mobile-medium-active-destination-label',
              ),
              maxLines: 3,
              overflow: TextOverflow.ellipsis,
          ...
          SizedBox(height: classicMobileTokens.spacingUnit * 1.5),
          Expanded(
            child: SingleChildScrollView(
              key: const ValueKey<String>(
                'classic-mobile-medium-navigation-scroll',
              ),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
...
final class _DestinationFocusAnchor extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Semantics(
    key: ValueKey<String>(
      'classic-mobile-destination-${data.activeDestination.name}',
    ),
    container: true,
    child: KeyedSubtree(
      key: ValueKey<String>('classic-mobile-focus-${data.initialFocusTarget}'),
