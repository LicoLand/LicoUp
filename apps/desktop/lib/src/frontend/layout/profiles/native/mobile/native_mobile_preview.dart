...
Widget buildNativeMobilePreview(BuildContext context) {
  return const NativeMobilePreview();
}

/// A deterministic rendering made only from public profile metadata and theme.
final class NativeMobilePreview extends StatelessWidget {
  const NativeMobilePreview({super.key});

  @override
  Widget build(BuildContext context) {
    final colors = context.layoutPalette;
    return Semantics(
      key: const Key('native-mobile-preview'),
      container: true,
      image: true,
          ...
          child: DecoratedBox(
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(
                NativeMobileMetrics.compactRadius,
              ),
              border: Border.all(color: colors.line, width: 1),
            ),
            child: ClipRRect(
              borderRadius: BorderRadius.circular(
                NativeMobileMetrics.compactRadius - 1,
              ),
              child: ExcludeSemantics(
                child: LayoutBuilder(
                  builder: (context, constraints) {
                    ...
                    return Row(
                      children: [
                        _NativePreviewRail(compact: compact),
                        Expanded(
                          child: _NativePreviewWorkspace(compact: compact),
                        ),
                      ],
                    );
                  },
...
final class _NativePreviewRail extends StatelessWidget {
  const _NativePreviewRail({required this.compact});
...
final class _NativePreviewWorkspace extends StatelessWidget {
  const _NativePreviewWorkspace({required this.compact});
...
final class _NativePreviewLine extends StatelessWidget {
  const _NativePreviewLine({
