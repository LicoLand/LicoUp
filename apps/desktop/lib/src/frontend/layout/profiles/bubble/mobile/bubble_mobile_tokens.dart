...
const bubbleMobileStyleIdentity = 'dense-docked-bubble';
const bubbleMobileRestorationPrefix = 'bubble.mobile';

final LayoutVisualTokens bubbleMobileVisualTokens = LayoutVisualTokens(
  spacingUnit: 4,
  density: 0.72,
  cardRadius: 5,
  elevation: 0,
...
/// Non-color measurements for the dense mobile Bubble presentation.
abstract final class BubbleMobileMetrics {
  ...
  static double compactHeaderExtentFor(double textScale) {
    if (!textScale.isFinite || textScale <= 0) {
      throw const FormatException('bubble_mobile_text_scale_invalid');
    }
  ...
  static Duration motion(LayoutEnvironment environment) {
    return environment.reducedMotion
        ? Duration.zero
        : bubbleMobileVisualTokens.motionDuration;
  }
}
