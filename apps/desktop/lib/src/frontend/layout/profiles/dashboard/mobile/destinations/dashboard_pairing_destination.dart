...
Widget buildDashboardPairingDestination(
  ...
  return RestorationScope(
    restorationId: '$dashboardMobileRestorationPrefix.mobile-relay.content',
    child: const DashboardMobileComponentKit().card(
      context,
      key: const ValueKey<String>('dashboard-mobile-pairing-card'),
      child: KeyedSubtree(
        key: const ValueKey<String>('dashboard-mobile-pairing-content'),
        child: content,
      ),
    ),
  );
...
void _verifyPairingContract(LayoutDestinationBuildContext data) {
  if (data.destination != ClientSection.mobileRelay ||
    ...
    throw const FormatException(
      'dashboard_mobile_pairing_destination_contract_invalid',
    );
  }
}
