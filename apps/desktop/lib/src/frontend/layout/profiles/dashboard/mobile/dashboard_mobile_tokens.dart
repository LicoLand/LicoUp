...
const dashboardMobileStyleIdentity = 'dashboard-channel-chat';
const dashboardMobileRestorationPrefix = 'dashboard.mobile';

final LayoutVisualTokens dashboardMobileVisualTokens = LayoutVisualTokens(
  spacingUnit: 4,
  density: 0.74,
  cardRadius: 6,
  elevation: 0,
...
/// Non-color measurements for the Dashboard mobile presentation.
abstract final class DashboardMobileMetrics {
  ...
  static double compactHeaderExtentFor(double textScale) {
    if (!textScale.isFinite || textScale <= 0) {
      throw const FormatException('dashboard_mobile_text_scale_invalid');
    }
  ...
  static Duration motion(LayoutEnvironment environment) {
    return environment.reducedMotion
        ? Duration.zero
        : dashboardMobileVisualTokens.motionDuration;
  }
}
