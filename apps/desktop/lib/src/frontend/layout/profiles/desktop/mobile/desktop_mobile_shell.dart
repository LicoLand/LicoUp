...
Widget buildDesktopMobileCompactShell(
  BuildContext context,
  LayoutShellBuildContext data,
) {
  _validateEnvironment(data, LayoutViewportClass.compact);
  return _DesktopMobileShell(data: data, compact: true);
}

Widget buildDesktopMobileMediumShell(
  BuildContext context,
  LayoutShellBuildContext data,
) {
  _validateEnvironment(data, LayoutViewportClass.medium);
  return _DesktopMobileShell(data: data, compact: false);
}
...
void _validateEnvironment(
  ...
  if (data.environment.surface != LayoutRuntimeSurface.mobile ||
    ...
    throw const FormatException('desktop_mobile_shell_contract_invalid');
  }
}

final class _DesktopMobileShell extends StatelessWidget {
  const _DesktopMobileShell({required this.data, required this.compact});
  ...
  Widget build(BuildContext context) {
    ...
    final media = (inheritedMedia ?? const MediaQueryData()).copyWith(
      textScaler: TextScaler.linear(
        DesktopMobileMetrics.boundedTextScale(environment),
      ),
      disableAnimations:
    ...
    return RestorationScope(
      restorationId:
          '$desktopMobileRestorationPrefix.${environment.viewport.name}.shell',
      child: MediaQuery(
        data: media,
  ...
  Widget _buildConstrainedShell(
    ...
    final adaptiveHorizontalPadding = math.min(
      DesktopMobileMetrics.horizontalPadding(environment),
      math.max(desktopMobileTokens.spacingUnit, constraintWidth * 0.06),
    );
    ...
    final boundedContentWidth = math.min(
      desktopMobileTokens.contentMaxWidth,
      contentWidth,
    );
    final topPadding =
        safeInsets.top +
    ...
    return ColoredBox(
      key: ValueKey<String>(
        'desktop-mobile-${environment.viewport.name}-shell',
      ),
        ...
        child: Align(
          ...
          child: ConstrainedBox(
            ...
            child: AnimatedPadding(
              ...
              child: compact
                  ? _DesktopCompactComposition(data: data)
                  : _DesktopMediumComposition(
                      data: data,
...
final class _DesktopCompactComposition extends StatelessWidget {
  const _DesktopCompactComposition({required this.data});

  final LayoutShellBuildContext data;

  @override
  Widget build(BuildContext context) => Column(
    key: const ValueKey<String>('desktop-mobile-compact-card-stack'),
    children: [
      _CompactContextualNavigation(data: data),
      const SizedBox(height: DesktopMobileMetrics.compactStackGap),
      Expanded(
        child: data.components.panel(
          context,
          key: const ValueKey<String>(
            'desktop-mobile-compact-destination-panel',
          ),
          emphasized: true,
...
final class _CompactContextualNavigation extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final interactiveExtent = DesktopMobileMetrics.interactiveExtent(
      environment,
    );
    ...
    return data.components.panel(
      context,
      key: const ValueKey<String>(
        'desktop-mobile-compact-contextual-navigation',
      ),
      child: Semantics(
        container: true,
        ...
        child: ConstrainedBox(
          ...
          child: Row(
            children: [
                ...
                child: DecoratedBox(
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(
                      desktopMobileTokens.cardRadius * 0.6,
                    ),
                  ),
                  child: Padding(
              ...
              SizedBox(width: desktopMobileTokens.spacingUnit * 1.5),
              Expanded(
                child: Text(
                  activeLabel,
                  key: const ValueKey<String>(
                    'desktop-mobile-active-destination-label',
                  ),
                  maxLines: 2,
              ...
              SizedBox(width: desktopMobileTokens.spacingUnit),
              Focus(
                canRequestFocus:
                  ...
                  itemBuilder: (context) => [
                    for (final destination in data.availableDestinations)
                      PopupMenuItem<ClientSection>(
                        key: ValueKey<String>(
                          'desktop-mobile-compact-navigation-${destination.name}',
                        ),
                        value: destination,
                        ...
                        child: Semantics(
                          ...
                          child: Row(
                            children: [
                              ...
                              SizedBox(
                                width: desktopMobileTokens.spacingUnit * 1.5,
                              ),
                              Expanded(
                                child: Text(
...
final class _DesktopMediumComposition extends StatelessWidget {
  const _DesktopMediumComposition({
    required this.data,
  ...
  Widget build(BuildContext context) {
    ...
    final scaledNavigationWidth =
        DesktopMobileMetrics.mediumNavigationWidth +
        (DesktopMobileMetrics.boundedTextScale(environment) - 1) * 28;
    ...
    return Row(
      key: const ValueKey<String>('desktop-mobile-medium-card-stack'),
      ...
      children: [
        ...
        const SizedBox(width: DesktopMobileMetrics.mediumStackGap),
        Expanded(
          child: data.components.panel(
            context,
            key: const ValueKey<String>(
              'desktop-mobile-medium-destination-panel',
            ),
            emphasized: true,
            child: _DestinationFocusAnchor(data: data),
          ),
...
final class _MediumContextualNavigation extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return data.components.panel(
      context,
      key: const ValueKey<String>(
        'desktop-mobile-medium-contextual-navigation',
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Semantics(
            ...
            child: Text(
              ...
              key: const ValueKey<String>(
                'desktop-mobile-medium-active-destination-label',
              ),
              maxLines: 3,
              overflow: TextOverflow.ellipsis,
          ...
          SizedBox(height: desktopMobileTokens.spacingUnit * 1.5),
          Expanded(
            child: SingleChildScrollView(
              key: const ValueKey<String>(
                'desktop-mobile-medium-navigation-scroll',
              ),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  for (final destination in data.availableDestinations) ...[
                    ...
                    if (destination != data.availableDestinations.last)
                      SizedBox(height: desktopMobileTokens.spacingUnit),
                  ],
                ],
              ),
            ),
...
final class _DestinationFocusAnchor extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Semantics(
    key: ValueKey<String>(
      'desktop-mobile-destination-${data.activeDestination.name}',
    ),
    container: true,
    child: KeyedSubtree(
      key: ValueKey<String>(
