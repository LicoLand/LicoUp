...
final class BubbleActionRecorder {
  ...
  void invokeContentAction(ClientSection destination, String action) {
    _requireDesktopDestination(destination);
    if (action != 'primary') {
      throw const FormatException('bubble_test_action_unknown');
    }
    contentActions.add((destination, action));
  }

  static void _requireDesktopDestination(ClientSection destination) {
    if (!bubbleDesktopExpectedDestinations.contains(destination)) {
      throw const FormatException('bubble_test_destination_unknown');
    }
  }
}

final class BubbleRecordingContentPort implements LayoutDestinationContentPort {
  BubbleRecordingContentPort(this.actions);

  final BubbleActionRecorder actions;
  ...
  Widget buildDestination(BuildContext context, ClientSection destination) {
    if (!bubbleDesktopExpectedDestinations.contains(destination)) {
      throw const FormatException('bubble_test_content_destination_unknown');
    }
    buildCalls.add(destination);
    brightnesses.add(Theme.of(context).brightness);
    return _BubbleFakeDestinationSurface(
...
final class BubbleDesktopTestHarness extends StatelessWidget {
  const BubbleDesktopTestHarness({
    super.key,
  ...
  final BubbleRecordingContentPort content;
  final BubbleActionRecorder actions;
  ...
  Widget build(BuildContext context) {
    ...
    if (variant == null) {
      throw const FormatException('bubble_test_viewport_unregistered');
    }
    final destinationBuilder = variant.destinationBuilders[activeDestination];
    if (destinationBuilder == null) {
      throw const FormatException('bubble_test_destination_unregistered');
    }
    ...
    return MaterialApp(
      ...
      home: Scaffold(
        body: Align(
          ...
          child: SizedBox(
            key: const ValueKey<String>('bubble-desktop-test-viewport'),
                    ...
                    builder: (profileContext) {
                      ...
                      return variant.shellBuilder(
                        ...
                        LayoutShellBuildContext(
                          ...
                          destinationLabel: bubbleDesktopDestinationLabel,
...
LayoutPalette bubbleDesktopTestPalette(ThemeData theme) {
  final colors = theme.extension<LicoThemeColors>();
  if (colors == null) {
    throw StateError('bubble_desktop_test_palette_missing');
  }
...
final class BubbleRecordingChromePort
    extends ValueNotifier<LayoutChromeSnapshot>
    implements LayoutChromePort {
  BubbleRecordingChromePort([super.value = const LayoutChromeSnapshot.empty()]);

  int pairingInvocations = 0;

  @override
  Future<void> openPairing(BuildContext context) async {
    pairingInvocations += 1;
  }
...
String bubbleDesktopDestinationLabel(ClientSection destination) {
  final label = bubbleDesktopTestLabels[destination];
  if (label == null) {
    throw const FormatException('bubble_test_destination_label_unknown');
  }
  return label;
}

LayoutEnvironment bubbleDesktopEnvironment({
...
void configureBubbleTestView(WidgetTester tester, Size size) {
...
final class _BubbleFakeDestinationSurface extends StatelessWidget {
  const _BubbleFakeDestinationSurface({
  ...
  Widget build(BuildContext context) {
    ...
    final label = bubbleDesktopDestinationLabel(destination);
    return Semantics(
      container: true,
      explicitChildNodes: true,
      label: 'Content $label',
      child: ColoredBox(
        key: ValueKey<String>('bubble-fake-content-${destination.name}'),
            ...
            children: [
              Row(
                children: [
                  ...
                  TextButton(
                    key: ValueKey<String>(
                      'bubble-content-action-${destination.name}',
                    ),
