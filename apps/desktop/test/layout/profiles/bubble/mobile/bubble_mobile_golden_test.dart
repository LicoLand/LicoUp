...
void main() {
  for (final brightness in Brightness.values) {
    testWidgets(
      'Bubble mobile preview is deterministic for ${brightness.name}',
      (tester) async {
              ...
              child: ColoredBox(
                ...
                child: Center(
                  child: RepaintBoundary(
                    key: const Key('bubble-mobile-preview-frame'),
                    child: SizedBox(
                      width: 336,
                      child: Builder(
                        builder: bubbleMobileBundle.previewBuilder,
                      ),
                    ),
                  ),
                ),
        ...
        await expectLater(
          find.byKey(const Key('bubble-mobile-preview-frame')),
          matchesGoldenFile(
            'goldens/bubble_mobile_preview_${brightness.name}.png',
          ),
        );
      },
    );
