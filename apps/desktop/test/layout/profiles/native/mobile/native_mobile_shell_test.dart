...
void main() {
  testWidgets('compact uses a contextual overlay with semantic navigation', (
    ...
    expect(
      find.byKey(const Key('native-mobile-compact-shell')),
      findsOneWidget,
    );
    expect(find.byKey(const Key('native-mobile-medium-rail')), findsNothing);
    expect(
      find.byKey(const Key('native-mobile-navigation-overlay')),
      findsNothing,
    );

    await tester.tap(find.byKey(const Key('native-mobile-menu-button')));
    ...
    expect(
      find.byKey(const Key('native-mobile-contextual-drawer')),
      findsOneWidget,
    );
    for (final destination in nativeMobileDestinations) {
      expect(
        find.byKey(
          ValueKey('native-mobile-compact-navigation-${destination.name}'),
        ),
        findsOneWidget,
      );
    }
    ...
    final agents = find.byKey(
      const ValueKey('native-mobile-compact-navigation-agents'),
    );
    ...
    await tester.tap(
      find.byKey(
        const ValueKey('native-mobile-compact-navigation-mobileRelay'),
      ),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('medium uses a narrow rail with equivalent destinations', (
    ...
    expect(find.byKey(const Key('native-mobile-medium-shell')), findsOneWidget);
    expect(find.byKey(const Key('native-mobile-medium-rail')), findsOneWidget);
    expect(find.byKey(const Key('native-mobile-menu-button')), findsNothing);
    for (final destination in nativeMobileDestinations) {
      expect(
        find.byKey(
          ValueKey('native-mobile-medium-navigation-${destination.name}'),
        ),
        findsOneWidget,
      );
    }
    final settings = find.byKey(
      const ValueKey('native-mobile-medium-navigation-settings'),
    );
    ...
    await tester.tap(
      find.byKey(const ValueKey('native-mobile-medium-navigation-agents')),
    );
    await tester.pump();
  ...
  testWidgets('keyboard traversal can focus and activate compact navigation', (
    ...
    await pumpNativeMobileHarness(
      tester,
      harness: harness,
      environment: nativeMobileEnvironment(hasTouch: false, hasKeyboard: true),
    );
  ...
  testWidgets('safe and keyboard insets keep composer and navigation clear', (
    ...
    final header = find.byKey(const Key('native-mobile-compact-header'));
    expect(tester.getTopLeft(header), const Offset(13, 17));
    final composer = find.byKey(const ValueKey('native-fake-composer-agents'));
    expect(tester.getBottomRight(composer).dy, lessThanOrEqualTo(420));

    await tester.tap(find.byKey(const Key('native-mobile-menu-button')));
    await tester.pumpAndSettle();
    final barrier = find.byKey(const Key('native-mobile-overlay-barrier'));
  ...
  testWidgets('large text remains overflow-safe in compact and medium', (
    ...
    final compactHarness = NativeMobileHarness();
    await pumpNativeMobileHarness(
      tester,
    ...
    final mediumHarness = NativeMobileHarness();
    await pumpNativeMobileHarness(
      tester,
  ...
  testWidgets('reduced motion resolves all Native shell transitions to zero', (
    tester,
  ) async {
    final harness = NativeMobileHarness();
    await pumpNativeMobileHarness(
      tester,
      harness: harness,
      environment: nativeMobileEnvironment(reducedMotion: true),
    );
  ...
  testWidgets('compact overlay state restores inside the Native namespace', (
    tester,
  ) async {
    final harness = NativeMobileHarness();
    await pumpNativeMobileHarness(
