...
void main() {
  testWidgets('preview is deterministic', (tester) async {
      ...
      MaterialApp(
        ...
        home: Center(
          child: SizedBox(
            width: 512,
            height: 320,
            child: Builder(builder: dashboardDesktopBundle.previewBuilder),
          ),
        ),
      ),
    );
    ...
    await expectLater(
      find.byKey(const ValueKey<String>('dashboard-desktop-preview')),
      matchesGoldenFile(
        '../../../../goldens/layout/dashboard/desktop/preview.png',
      ),
    );
  });

  testWidgets('medium shell has a stable dashboard composition', (
    tester,
  ) async {
  ...
  testWidgets('expanded shell has a stable dashboard composition', (
    tester,
  ) async {
    await _expectShellGolden(
      tester,
...
Future<void> _expectShellGolden(
  ...
  await tester.pumpWidget(
    DashboardDesktopShellHarness(
      environment: dashboardDesktopEnvironment(
        width: size.width,
        height: size.height,
      ),
  ...
  await expectLater(
    ...
    matchesGoldenFile('../../../../goldens/layout/dashboard/desktop/$fileName'),
  );
}
