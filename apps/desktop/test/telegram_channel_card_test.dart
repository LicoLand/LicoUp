...
void main() {
  testWidgets('renders status, token actions, and empty pairing sections', (
    ...
    expect(
      find.byKey(const Key('telegram-channel-save-token')),
      findsOneWidget,
    );
    expect(
      find.byKey(const Key('telegram-channel-pairings-empty')),
      findsOneWidget,
    );
    expect(
      find.byKey(const Key('telegram-channel-chats-empty')),
      findsOneWidget,
    );
    expect(
  ...
  testWidgets('save token stops and starts gateway, then refreshes', (
    ...
    expect(
      jsonDecode(runner.stdinCalls.single.body)['botToken'],
      '123456:ABC-DEF',
    );
    expect(
      runner.calls.where(
  ...
  testWidgets('approve pending pairing and revoke approved chat', (
    ...
    final runner = _FakeTelegramRunner()
      ..configured = true
      ..pairings = [
        {'code': 'ABCD12', 'chatId': 11, 'userId': 22, 'username': 'alice'},
      ]
      ..chats = [
...
final class _FakeTelegramRunner implements AgentCommandRunner {
  ...
  Future<Map<String, dynamic>> runCli(List<String> args) async {
    ...
    if (args.join(' ') == 'gateway channel telegram credentials status') {
      return {
        'ok': true,
    ...
    if (args.join(' ') == 'gateway channel telegram pairing list') {
      return {'ok': true, 'pairings': pairings, 'chats': chats};
    }
    if (args.length >= 5 &&
        args[0] == 'gateway' &&
        args[3] == 'pairing' &&
    ...
    if (args.join(' ') == 'gateway channel telegram credentials clear') {
      configured = false;
