...
List<TargetCandidate> _routingStrategyTargets() {
  return [
...
_createRoutingStrategyHarness({
  required String strategy,
  required _FakeAgentService service,
}) async {
  ...
  service.scanTargetsResult = _routingStrategyTargets();
...
Future<void> _waitForRuntimeMessageCount(
...
void main() {
  ...
  test(
    'phone assistant guard requires the selected account and its exact grant',
    () async {
      final directory = await Directory.systemTemp.createTemp(
        'lico-mobile-assistant-grants-',
      );
      ...
      await controller.configureMobileAgentApiKey(
        providerId: 'deepseek',
        apiKey: ['assistant-guard', '-fixture-0001'].join(),
      );
      await controller.addMobileAgentProvider('deepseek');
      final first = controller.mobileAgentAccounts.first;
      final second = controller.mobileAgentAccounts.last;
      expect(first.id, isNot(second.id));
      await controller.configureMobileAgentApiKey(
        providerId: 'deepseek',
        mobileAccountId: second.id,
        apiKey: ['assistant-guard', '-fixture-0002'].join(),
      );
      await controller.updateMobileAgentAssistantGrants(
        accountId: first.id,
        grants: const MobileAgentAssistantGrants(localInfo: true),
      );

      await controller.setActiveMobileAgentAccount(second.id);
      expect(
      ...
      expect(
        controller.guardMobileAgentAssistantAction(
          accountId: first.id,
          action: 'local-info',
        ),
        isFalse,
      ...
      await controller.setActiveMobileAgentAccount(first.id);
      expect(
        controller.guardMobileAgentAssistantAction(
          accountId: first.id,
          action: 'local-info',
  ...
  test(
    'refreshes one mobile API credential without touching siblings',
    () async {
      final directory = await Directory.systemTemp.createTemp(
        'lico-mobile-account-refresh-',
      );
      addTearDown(() => _deleteTempDirectory(directory));
      ...
      await controller.configureMobileAgentApiKey(
        providerId: 'deepseek',
        apiKey: ['account-refresh', '-fixture-0001'].join(),
      );
      final account = controller.mobileAgentAccounts.single;
      relayService.credentialStatusPresent = false;
      await controller.refreshMobileAgentAccountStatus(account);

      expect(relayService.credentialStatusCalls, 1);
      expect(controller.mobileAgentAccounts.single.credentialPresent, isFalse);
      expect(controller.lastError, isEmpty);

      relayService.credentialStatusPresent = true;
      await controller.refreshMobileAgentAccountStatus(
        controller.mobileAgentAccounts.single,
      );
      expect(relayService.credentialStatusCalls, 2);
      expect(controller.mobileAgentAccounts.single.credentialPresent, isTrue);
    },
  );
  ...
  test(
    ...
    () async {
      ...
      await controller.addMobileAgentProvider('gemini');
      await controller.addMobileAgentProvider('deepseek');
      final geminiAccountId = controller.mobileAgentAccounts
          .singleWhere((account) => account.providerId == 'gemini')
          .id;
      final deepSeekAccountId = controller.mobileAgentAccounts
          .singleWhere((account) => account.providerId == 'deepseek')
          .id;
      final geminiEntryId = 'account:$geminiAccountId';
      await controller.toggleMobileHomeEntryPinned(geminiEntryId);
      await controller.reorderMobileHomePinnedEntries([geminiEntryId], 0, 0);
      ...
      expect(
        ...
        contains(geminiEntryId),
      );

      await controller.deleteMobileAgentAccounts([geminiAccountId]);
      ...
      expect(
        controller.mobileHomeLayout.pinnedEntryIds,
        isNot(contains(geminiEntryId)),
      );
      expect(controller.mobileHomeLayout.order, isNot(contains(geminiEntryId)));
...
''', flush: true);
  ...
  test(
    ...
    () async {
      ...
      expect(
        await File(
          p.join(directory.path, 'lico-client', 'agent-tab-order.json'),
        ).exists(),
        isTrue,
      );
      expect(
  ...
  test(
    ...
    () async {
      final sessions = List<Map<String, dynamic>>.generate(
        25,
        (index) => _conversationSessionJson(
  ...
  test(
    ...
    () async {
      final exact = _conversationSessionJson(
        id: 'codex-projection-exact',
        nativeSessionId: 'codex-native-exact',
        agentId: 'codex',
  ...
  test('conversation refresh priority follows view attention', () {
    final controller = ClientController(agentService: _FakeAgentService());
    addTearDown(controller.dispose);
    controller.currentSection = ClientSection.agents;
  ...
  test(
    ...
    () async {
      ...
      expect(controller.agentOrchestrationCircuitStates, isEmpty);
      expect(controller.agentOrchestrationOpenCircuitAgentIds, isEmpty);
      expect(
        controller.selectedConversationAgentId,
        agentOrchestrationTargetId,
      );
  ...
  test(
    ...
    () async {
      final service = _FakeAgentService();
      final harness = await _createRoutingStrategyHarness(
        strategy: 'serial-all',
        service: service,
      );
      addTearDown(harness.controller.dispose);
      addTearDown(() => _deleteTempDirectory(harness.directory));
  ...
  test(
    ...
    () async {
      final gate = Completer<void>();
      final service = _FakeAgentService()..runtimeMessageGate = gate;
      final harness = await _createRoutingStrategyHarness(
      ...
      await _waitForRuntimeMessageCount(service, 3);
  ...
  test(
    ...
    () async {
      ...
      expect(controller.displayStatusMessage, contains(directory.path));
  ...
  test('mobile ChatGPT OAuth Android bridge uses ChatGPT Codex responses', () {
    ...
    expect(bridge, contains('isDeferredAndroidMobileProvider(providerId)'));
    expect(bridge, contains('"local_oauth_start"'));
    expect(bridge, contains('"local_oauth_callback"'));
    expect(bridge, contains('"local_oauth_status"'));
    expect(bridge, contains('"local_credential_set"'));
    expect(bridge, contains('"local_credential_status"'));
    expect(bridge, contains('"local_credential_delete"'));
    expect(bridge, contains('"android_provider_deferred"'));
  ...
  test(
    ...
    () async {
      ...
      // Only API-key providers sync credentials into the phone secure store.
      // ChatGPT uses its explicit OAuth path. Gemini/Kimi local authorization
      // is deferred, so those optional providers remain desktop-relay echoes.
      expect(relayService.syncedProviderIds, ['deepseek']);
      expect(
      ...
      expect(syncedApiKeyAccounts.map((account) => account.providerId), [
        'deepseek',
      ]);
      expect(
        controller.mobileAgentAccounts
            .where((account) => account.providerId == 'chatgpt')
            .every((account) => account.usesDesktopRelay),
        isTrue,
      );
      expect(
        controller.mobileAgentAccounts
...
class _FakeAgentService extends AgentService {
  ...
  Stream<Map<String, dynamic>> streamCliJsonLines(List<String> args) async* {
    ...
    if (args.length >= 2 && args[0] == 'conversations' && args[1] == 'stream') {
      ...
      final gate = conversationStreamGates[_argValue(args, '--agent')];
      if (gate != null) {
        await gate.future;
      }
...
class _FakeMobileRelayService extends MobileRelayService {
  ...
  bool credentialStatusPresent = true;
    ...
    config = config.copyWith(
      ...
      pairedDevices: [
        MobileRelayPairedDevice(
          ...
          mobileToken: 'mobile-token',
          credentialPresent: true,
          gatewayUrl: (invite['gatewayUrl'] ?? licoDefaultMobileRelayGatewayUrl)
              .toString(),
        ),
      ],
    );
  ...
  Future<Map<String, dynamic>> mobileProviderCredentialStatus({
    ...
    return {
      ...
      'providerId': providerId,
      'mobileAccountId': mobileAccountId,
      'credentialPresent': credentialStatusPresent,
      'credentialHint': credentialStatusPresent ? '**** 0001' : '',
      'bodyRedacted': true,
    };
  }
