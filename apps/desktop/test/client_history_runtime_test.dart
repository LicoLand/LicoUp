...
void main() {
  ...
  test('history runtime scenarios are behavior-grouped without losing coverage', () {
    ...
    for (final entry in groupSources.entries) {
      ...
      expect(
        ...
        reason: '${entry.key} must own exactly its declared behavior scenarios',
      );
      expect(source, isNot(contains(RegExp(r'class\s+FakeAgentService\b'))));
      expect(
        source,
        isNot(contains(RegExp(r'Map<String,\s*dynamic>\s+conversationSessionJson\s*\('))),
      );
    }
    ...
    final allDeclarations = declarationsByGroup.values.expand((value) => value);
    ...
    final fixtureSources = fixtureRoot
        ...
        .toList();
    expect(
      fixtureSources.where(
        (entry) => RegExp(r'class\s+FakeAgentService\b').hasMatch(entry.value),
      ).map((entry) => entry.key),
...
Directory _desktopPackageRoot() {
  ...
  while (true) {
    if (File('${candidate.path}/pubspec.yaml').existsSync() &&
        Directory('${candidate.path}/test/fixtures/client_controller').existsSync()) {
      return candidate;
    }
...
String _normalized(String source) => source.replaceAll(RegExp(r'\s+'), ' ').trim();
...
int? _closureBodyOpen(String source, int start) {
  ...
  if (cursor + 1 >= source.length || source.substring(cursor, cursor + 2) != '()') {
    return null;
  }
