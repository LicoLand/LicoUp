...
const checkNames = [
  ...
  "selectedTargetGatesReady", "selectedTargetBuilt", "archiveLayoutReady",
...
function receipt() {
  return {
    schemaVersion: "licoup.release-pre-pr-receipt.v1",
    sourceRevision,
    sourceTree,
    version: "1.2.3",
    target: "macos-arm64",
...
test("release template binds the one pre-PR authority", () => {
...
test("receipt is strict, source-bound, and privacy-safe", () => {
  ...
  for (const mutate of [
    ...
    (value) => { value.target = "android-arm64"; },
...
test("pre-push parser is bounded and target stages include real closure", () => {
  ...
  assert.deepEqual(targetStages("macos-arm64").map((entry) => entry[1]), [
    "client:build:macos",
    "client:install:macos:identity",
    "client:archive:macos-github-release",
    "client:verify:macos-release-artifact",
    "client:verify:macos-update-preflight",
  ]);
});
