...
use crate::platform::process_identity;
...
const DEFAULT_LOCAL_RUNTIME_ROOT: &str = ".licolite-server-data";
const LOCAL_RUNTIME_ROOT_ENV: &str = "LICO_LOCAL_RUNTIME_DIR";
...
fn local_runtime_root() -> Result<PathBuf> {
    if let Some(portable_override) = crate::platform::paths::portable_data_dir_override_path() {
        return Ok(portable_override
            .join("lico-client")
            .join(LOCAL_RUNTIME_DIR));
    }
    if let Some(path) = local_runtime_root_from_env() {
        return Ok(path);
    }
    let base_dirs = directories::BaseDirs::new()
        .ok_or_else(|| anyhow!("cannot resolve home directory for local runtime"))?;
    Ok(base_dirs.home_dir().join(DEFAULT_LOCAL_RUNTIME_ROOT))
}

fn local_runtime_root_from_env() -> Option<PathBuf> {
    let raw = env::var(LOCAL_RUNTIME_ROOT_ENV).ok()?;
...
fn write_runtime_config(
    ...
    let config = json!({
        ...
        "discovery": {
            ...
            "serverLabel": "Lico Arc Local Runtime",
...
mod tests {
    ...
    #[test]
    fn runtime_paths_default_to_licolite_server_data_when_not_overridden() {
        if env::var(LOCAL_RUNTIME_ROOT_ENV).is_ok() {
            return;
        }
        let previous = set_portable_data_dir_override(None);
        let root = local_runtime_root().unwrap();
        assert_eq!(
            root.file_name().and_then(|value| value.to_str()),
            Some(DEFAULT_LOCAL_RUNTIME_ROOT)
        );
        assert!(!root.ends_with(LOCAL_RUNTIME_DIR));
        set_portable_data_dir_override(previous);
    }

    #[test]
    fn runtime_paths_keep_portable_override_for_tests() {
        let temp = env::temp_dir().join(format!("lico-local-runtime-override-{}", unix_nanos()));
        fs::create_dir_all(&temp).unwrap();
        let previous = set_portable_data_dir_override(Some(temp.clone()));
        let paths = runtime_paths().unwrap();
        assert_eq!(paths.root, temp.join("lico-client").join(LOCAL_RUNTIME_DIR));
        set_portable_data_dir_override(previous);
    }
