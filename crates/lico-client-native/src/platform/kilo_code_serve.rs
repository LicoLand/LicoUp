//! Kilo Code headless `serve` lifecycle owned by the native client.
//!
//! Kilo mirrors OpenCode's serve/attach HTTP lane. Arc auto-starts a localhost
//! serve on ports 4097–4116 (avoiding vendor example 4096 and Arc/LicoLite
//! reserved ports), and persists the attach endpoint so exact-session
//! open/send can reuse the running environment. ACP remains a secondary lane.
...
/// Default Kilo serve port. Vendor attach examples use 4096; Arc owns 4097–4116.
pub const DEFAULT_PORT: u16 = 4097;
const PORT_RANGE_SPAN: u16 = 19;
const HEALTH_PATH: &str = "/global/health";
const SESSION_PROBE_PATH: &str = "/session";
const DEFAULT_HOST: &str = "127.0.0.1";
const STATE_DIR: &str = "kilo-code-serve";
const DEFAULT_HEALTH_TIMEOUT_MS: u64 = 45_000;
const STATE_SCHEMA_VERSION: &str = "v0.0.1:kilo-code-serve-1";

/// Ports reserved by Arc / LicoLite / vendor defaults that must not be selected
/// even when currently free. Includes research-locked exclusions:
/// 4096 (OpenCode attach example), 17328 (local-runtime), 18789 (OpenClaw
/// vendor default), 58627 (Arc reserved).
    ...
    fn spawn_serve(&self, executable: &str, host: &str, port: u16, log_path: &Path) -> Result<u32> {
        ...
        let child = command.spawn().map_err(|error| match error.kind() {
            std::io::ErrorKind::NotFound => {
                anyhow!("kilo executable is not available")
            }
            std::io::ErrorKind::PermissionDenied => {
                anyhow!("kilo executable is not permitted to run")
            }
            _ => anyhow!("kilo serve could not be started"),
        })?;
        Ok(child.id())
    }
}
...
fn install_test_runner(runner: Box<dyn ServeRunner>) {
    *runner_slot().lock().expect("kilo serve runner lock") = Some(runner);
}

#[cfg(test)]
fn clear_test_runner() {
    *runner_slot().lock().expect("kilo serve runner lock") = None;
}
...
fn active_runner() -> Box<dyn ServeRunner> {
    ...
    impl ServeRunner for Delegating {
        fn spawn_serve(
            ...
            let guard = runner_slot().lock().expect("kilo serve runner lock");
...
pub fn ensure_attach_endpoint(executable: &str) -> Result<ServeEndpoint> {
        ...
        || result.get("healthy").and_then(Value::as_bool) != Some(true)
    {
        let code = result
            .get("errorCode")
            .and_then(Value::as_str)
            .unwrap_or("kilo_code_serve_unavailable");
    ...
    F: Fn(u16) -> bool,
{
    ...
    Err(anyhow!("kilo_code_serve_port_exhausted"))
}
...
fn start_with_paths(params: &Value, paths: &ServePaths, reuse_healthy: bool) -> Result<Value> {
    ...
    if host != DEFAULT_HOST && host != "localhost" {
        ...
        return Err(anyhow!(
            "kilo serve auto-start only binds localhost; pass attachUrl for a remote endpoint"
        ));
    }
    ...
    if !executable_available(&executable) {
        ...
        let state = json!({
            ...
            "status": "unavailable",
            "running": false,
            "errorCode": "kilo_executable_missing",
    ...
    // If preferred is occupied by a non-Kilo listener, select_available_port
    // will skip it via bind failure. If preferred is reserved, also skip.
    let selected = match select_available_port(preferred) {
        Ok(port) => port,
        Err(_) => {
            // Preferred may be occupied by a healthy Kilo we can still attach to.
            if let Ok(health) = one_health_check(&preferred_endpoint.attach_url)
                && health_payload_is_ready(Some(&health))
            {
                return persist_reused_endpoint(
            ...
            let state = json!({
                ...
                "status": "blocked",
                "running": false,
                "errorCode": "kilo_code_serve_port_exhausted",
                "preferredPort": preferred,
                "host": host,
                "port": preferred,
                "attachUrl": preferred_endpoint.attach_url,
    ...
    let pid = match runner.spawn_serve(&executable, &host, selected, &paths.log_path) {
        ...
        Err(error) => {
            ...
            return Ok(json!({
                ...
                "attachUrl": endpoint.attach_url,
                "portConflict": port_conflict,
                "executableAvailable": code != "kilo_executable_missing"
            }));
        }
    };
    ...
    match wait_for_health(&endpoint.attach_url, timeout) {
        ...
        Err(_) => {
            ...
            let state = json!({
                ...
                "status": "blocked",
                "running": false,
                "errorCode": "kilo_code_serve_health_failed",
                "preferredPort": preferred,
                "host": host,
                "port": selected,
                "attachUrl": endpoint.attach_url,
...
fn status_for_remote(attach_url: &str) -> Result<Value> {
    ...
    {
        return Err(anyhow!(
            "remote Kilo attach URL must use https:// or localhost http"
        ));
    }
    ...
    Ok(json!({
        ...
        "remote": true,
        "health": health.unwrap_or(Value::Null),
        "errorCode": if healthy { Value::Null } else { json!("kilo_code_serve_health_failed") }
    }))
}
...
fn verify_attach_ready(attach_url: &str) -> Result<()> {
    ...
    if !health_payload_is_ready(Some(&health)) {
        return Err(anyhow!("kilo_code_serve_health_failed"));
    }
    ...
    get_json(&sessions_url).map_err(|_| anyhow!("kilo_code_serve_attach_probe_failed"))?;
    Ok(())
}
...
pub(super) fn get_json(url: &str) -> Result<Value> {
    let response = ureq::AgentBuilder::new()
        ...
        .map_err(|error| match error {
            ureq::Error::Status(404, _) => anyhow!("kilo_code_serve_not_found"),
            _ => anyhow!("kilo serve request failed"),
        })?;
    response
        .into_json::<Value>()
        .map_err(|_| anyhow!("kilo serve returned invalid JSON"))
}
...
/// Watch Kilo serve SSE `/event` frames and forward assistant text chunks.
...
fn stop_process(paths: &ServePaths) -> Result<Value> {
        ...
        && process_alive(Some(pid))
    {
        ...
        if process_alive(Some(pid)) {
            return Err(anyhow!("failed to stop kilo serve process"));
        }
    }
...
fn resolve_executable(params: &Value) -> Result<String> {
    ...
    for key in ["KILO_BIN", "KILO_PATH", "KILOCODE_PATH"] {
...
fn read_state(path: &Path) -> Result<Value> {
    ...
    serde_json::from_str(&text).map_err(|_| anyhow!("kilo serve state is invalid"))
}
...
mod tests {
    ...
    impl ServeRunner for FakeRunner {
        fn spawn_serve(
            ...
            if self.fail {
                return Err(anyhow!("kilo executable is not available"));
            }
    ...
    fn reserved_ports_are_rejected_even_when_bindable() {
        ...
        assert_eq!(DEFAULT_PORT, 4097);
        assert!(!is_reserved_conflict_port(4116));
    }
    ...
    fn select_port_fails_closed_when_range_exhausted() {
        ...
        assert!(error.to_string().contains("kilo_code_serve_port_exhausted"));
    }

    #[test]
    fn ensure_fails_closed_when_executable_missing() {
        let dir = std::env::temp_dir().join(format!("lico-kilo-serve-missing-{}", unix_seconds()));
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        let previous = paths::set_portable_data_dir_override(Some(dir.clone()));
        clear_test_runner();
        // Prefer an unused port in the Arc-owned range so a leftover vendor
        // listener on 4096/4097 cannot be adopted as a healthy attach target.
        let result = ensure(&json!({
            "executable": dir.join("missing-kilo-binary").to_string_lossy(),
            "port": 4115,
            "healthTimeoutMs": 100
        }))
        .unwrap();
        assert_eq!(result["ok"], false);
        assert_eq!(result["errorCode"], "kilo_executable_missing");
    ...
    fn ensure_persists_port_conflict_when_preferred_is_reserved() {
        let dir = std::env::temp_dir().join(format!("lico-kilo-serve-conflict-{}", unix_seconds()));
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        let previous = paths::set_portable_data_dir_override(Some(dir.clone()));
        clear_test_runner();
        ...
        let fake_bin = dir.join("fake-kilo");
        ...
        .unwrap();
        assert_eq!(result["ok"], false);
        assert_eq!(result["errorCode"], "kilo_code_serve_health_failed");
