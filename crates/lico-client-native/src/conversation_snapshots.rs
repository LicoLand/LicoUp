...
use crate::file_security::{atomic_write_private_text, ensure_private_dir, harden_private_tree};
...
fn extract_tar_gz(bytes: &[u8], destination: &Path) -> Result<()> {
    let mut child = Command::new("tar")
        ...
        .stderr(Stdio::piped())
        .spawn()
        .map_err(|error| anyhow!("unable to start local tar extraction: {}", error))?;
...
fn validate_archive_collection(
    ...
    for record in index_records {
        ...
        for key in ["snapshot_path", "raw_content_path"] {
...
fn conversation_index_markdown(
    ...
    out.push_str("| Title | Source | Adapter | Confidence | Status | Path |\n");
    out.push_str("| --- | --- | --- | --- | --- | --- |\n");
    for record in index_records {
        out.push_str(&format!(
            "| {} | `{}` | `{}` | `{}` | `{}` | `{}` |\n",
...
fn invoke_preferred_curator(
    ...
    copy_preferred_runtime_field(&preferred, &mut runtime_params, "command");
    copy_preferred_runtime_field(&preferred, &mut runtime_params, "args");
    copy_preferred_runtime_field(&preferred, &mut runtime_params, "stdin");
...
fn client_state_store(params: &Value) -> Result<ClientStateStore> {
    ...
    if let Some(portable_dir) = text_param(params, &["portableDir"]) {
        if !portable_dir.is_empty() {
            return ClientStateStore::new(expand_home(&portable_dir).join("future-client"));
        }
    }
    Ok(ClientStateStore::new(
        portable_data_dir()?.join("future-client"),
    )?)
}
...
mod tests {
    ...
    fn preferred_curator_runtime_output_can_select_non_matching_candidate() {
        ...
        #[cfg(windows)]
        let script = {
            let output = home.join("curator-output.json");
            fs::write(&output, &curation_json).unwrap();
            let script = home.join("curator.cmd");
            fs::write(
                &script,
                format!("@echo off\r\ntype \"{}\"\r\n", output.display()),
            )
        ...
        #[cfg(not(windows))]
        let script = {
            let script = home.join("curator.sh");
            fs::write(
                &script,
                format!("#!/bin/sh\ncat <<'JSON'\n{}\nJSON\n", curation_json),
            )
