...
use crate::conversation_snapshots;
...
impl ArchiveJobStore {
    fn from_params(params: &Value) -> Result<Self> {
        let root = if let Some(state_root) = text_param(params, &["stateRoot", "clientStateRoot"]) {
            ...
            ClientStateStore::new(expand_home(&portable_dir).join("future-client"))?
                .root()
                .join(ARCHIVE_JOB_DIR)
        } else {
            ClientStateStore::new(portable_data_dir()?.join("future-client"))?
                .root()
                .join(ARCHIVE_JOB_DIR)
        };
        Self::new(root)
...
mod tests {
    ...
    fn create_job_persists_target_scan_and_queued_state() {
        ...
        assert_eq!(
            result["targetScan"]["candidates"].as_array().unwrap().len(),
            10
        );
        let history_path = display_path(&history);
        assert!(
            result["targetScan"]["candidates"]
                .as_array()
                .unwrap()
                .iter()
                .any(|candidate| candidate["target"] == "codex"
