...
fn handle_mcp_plugin(args: &[String]) -> Result<CliExecution> {
    ...
    let result = match action.as_str() {
        "status" => crate::mcp_plugins::plugin_status(&params)?,
        "update" => crate::mcp_plugins::plugin_update(&params)?,
        "rollback" => crate::mcp_plugins::plugin_rollback(&params)?,
...
fn handle_mcp_config(args: &[String]) -> Result<CliExecution> {
    ...
    let params = cli_params(&args[3..]);
    let result = match action.as_str() {
        "plan" => crate::targets::mcp_config_plan(&params)?,
        "apply" => crate::targets::mcp_config_apply(&params)?,
        "rollback" => crate::targets::mcp_config_rollback(&params)?,
        _ => return Ok(CliExecution::Usage),
    };
    Ok(CliExecution::Json(result))
}
