    ...
    match record.workflow_kind {
        WorkflowKind::LocalDeployment => {
            ...
            if let Some(assembly) = record.local_assembly.as_ref() {
                changes.push(json!({
                    "selectionId": "licoarc-assembly-manifest",
                    "sourceRelativePath": "licoarc-generated/assembly-manifest",
                    "destination": PathBuf::from(destination).join("licoarc-assembly.json"),
                    "destinationRelativePath": "licoarc-assembly.json",
