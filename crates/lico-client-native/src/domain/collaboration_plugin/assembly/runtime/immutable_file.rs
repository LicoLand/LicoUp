...
fn linux_memfd(bytes: &[u8], executable: bool) -> Result<ImmutableRuntimeFile> {
    ...
    let name = CString::new("licoarc-local-server-runtime")?;
    let raw = unsafe {
...
mod tests {
    ...
    fn copied_executable_is_immutable_and_survives_source_buffer_mutation() {
        let root =
            std::env::temp_dir().join(format!("licoarc-immutable-runtime-test-{}", Uuid::new_v4()));
    ...
    fn empty_runtime_image_is_rejected() {
        let root =
            std::env::temp_dir().join(format!("licoarc-empty-runtime-test-{}", Uuid::new_v4()));
        let store = ClientStateStore::new(root.clone()).unwrap();
