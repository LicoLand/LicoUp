...
const SCHEMA: &str = "licoarc.local-server-assembly-transaction.v1";
...
fn rollback_name(deployment_id: &str) -> String {
    format!(".licoarc-local-server-apply-rollback-{deployment_id}")
}
...
mod tests {
    ...
    fn fixture(name: &str) -> (std::path::PathBuf, ClientStateStore) {
        let root = std::env::temp_dir().join(format!(
            "licoarc-assembly-transaction-{name}-{}",
            Uuid::new_v4()
        ));
