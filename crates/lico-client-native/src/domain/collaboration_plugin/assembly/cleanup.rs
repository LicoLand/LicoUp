...
const SCHEMA: &str = "licoarc.local-server-assembly-cleanup.v1";
...
impl PendingAssemblyCleanup {
    pub(super) fn prepare(
        ...
        let quarantine = parent.join(format!(".licoarc-local-server-uninstall-{operation_id}"));
        let pending = Self {
    ...
    pub(super) fn validate(&self) -> Result<()> {
        ...
        let expected_name = format!(".licoarc-local-server-uninstall-{}", self.operation_id);
        ensure!(
