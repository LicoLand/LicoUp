...
fn plugin_fixture(name: &str) -> PathBuf {
    ...
    fs::write(
            ...
            br#"{"schemaVersion":"licoarc.collaboration.local-deployment.v1","manualOnly":true,"features":[{"id":"server-core","label":"Server Core","packagePath":"payload/server-core"}]}"#,
        )
        .unwrap();
    fs::write(
            root.join("workflows/mcp-install.json"),
            br#"{"schemaVersion":"licoarc.collaboration.mcp-install.v2","manualOnly":true,"plugins":[{"id":"selected","label":"Selected MCP","packagePath":"payload/mcp-selected","endpoint":"https://example.invalid/mcp"}],"requiresPerFileApproval":true,"outboundPolicy":"direct-user-exact-scope-one-shot"}"#,
        )
        .unwrap();
...
fn staged_package_mutation_invalidates_the_install_plan() {
    ...
    fs::write(
            ...
            br#"{"schemaVersion":"licoarc.collaboration.mcp-install.v2","manualOnly":true,"plugins":[{"id":"changed","label":"Changed MCP","packagePath":"payload/mcp-selected","endpoint":"https://example.invalid/changed"}],"requiresPerFileApproval":true,"outboundPolicy":"direct-user-exact-scope-one-shot"}"#,
        )
        .unwrap();
    assert!(
        install_apply_in(
