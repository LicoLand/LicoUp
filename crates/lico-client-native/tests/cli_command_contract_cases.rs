...
fn run_lico_client(args: &[String]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_lico-client"))
        ...
        .output()
        .expect("the real lico-client binary must be runnable")
}
...
fn run_lico_client_rpc(args: Vec<String>) -> Output {
    let mut child = Command::new(env!("CARGO_BIN_EXE_lico-client"))
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("the real lico-client RPC subprocess must start");
    let request = json!({
        "protocol": "lico-client.stdio.v1",
    ...
    child
        .wait_with_output()
        .expect("the real lico-client RPC subprocess must finish")
}
