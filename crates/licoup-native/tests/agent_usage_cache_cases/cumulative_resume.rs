...
fn update_usage(database: &PathBuf, input: i64, output: i64, last_seen: &str) {
    ...
    connection
        .execute(
            ...
            rusqlite::params![input, output, last_seen],
        )
        .unwrap();
}
...
fn cumulative_metadata_counts_new_usage_when_an_old_session_resumes() {
    ...
    let database = history_root.join("hermes.db");
    let connection = SqliteConnection::open(&database).unwrap();
    connection
        .execute_batch(
            "CREATE TABLE session_model_usage(
             ...
             INSERT INTO session_model_usage VALUES(
               'resumed-session','model-a',100,10,0,0,0,
               '2026-07-01T10:00:00Z','2026-07-10T09:00:00Z'
             );",
        )
        .unwrap();
    drop(connection);
    ...
    assert_eq!(rolled["summary"]["totalTokens"], 20);
    assert_eq!(
