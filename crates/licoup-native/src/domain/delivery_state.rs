//! Private current-generation delivery dispatch and runner-control records.
//!
//! Canonical Conversations own execution and resume state. This module stores
//! only delivery coordination and numeric usage-settlement metadata.
...
pub const DELIVERY_DISPATCH_SCHEMA_VERSION: &str = "licoup.delivery-dispatch.v1";
...
/// Numeric Token settlement lifecycle attached to the one current dispatch
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct DeliveryDispatchRecord {
    pub schema_version: String,
...
impl DeliveryDispatchRecord {
    /// Construct a current-generation delivery dispatch without exposing a
    ...
    pub fn new_delivery(
        ...
        Self {
            schema_version: DELIVERY_DISPATCH_SCHEMA_VERSION.to_owned(),
            generation: 1,
            ...
            state: DeliveryDispatchState::Accepted,
            session_mode,
            main_conversation_path,
            conversation_path: None,
            manager_conversation_binding,
            child_conversation_binding,
            lineage_scope,
...
pub fn delivery_dispatch_root(portable_data: &Path) -> PathBuf {
    portable_data
        .join("client-state")
        .join("delivery-state")
        .join("dispatches")
...
pub fn delivery_dispatch_path(portable_data: &Path, dispatch_id: &str) -> PathBuf {
    delivery_dispatch_root(portable_data).join(format!("{dispatch_id}.json"))
}
...
pub fn persist_delivery_dispatch(
    portable_data: &Path,
    record: &DeliveryDispatchRecord,
) -> Result<(), String> {
    if !valid_dispatch_id(&record.dispatch_id) {
        return Err("delivery_dispatch_dispatch_id_invalid".to_owned());
    }
    let root = delivery_dispatch_root(portable_data);
    fs::create_dir_all(&root).map_err(|_| "delivery_dispatch_store_unavailable".to_owned())?;
    let root_metadata = fs::symlink_metadata(&root)
        .map_err(|_| "delivery_dispatch_store_unavailable".to_owned())?;
    if root_metadata.file_type().is_symlink() || !root_metadata.is_dir() {
        return Err("delivery_dispatch_store_invalid".to_owned());
    }
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
    ...
    let path = delivery_dispatch_path(portable_data, &record.dispatch_id);
    if path.exists()
        && fs::symlink_metadata(&path)
            .map_err(|_| "delivery_dispatch_store_unavailable".to_owned())?
            .file_type()
    ...
    let body = serde_json::to_vec_pretty(record)
        .map_err(|_| "delivery_dispatch_encode_failed".to_owned())?;
    ...
    let mut file = options
        .open(&tmp)
        .map_err(|_| "delivery_dispatch_write_failed".to_owned())?;
    file.write_all(&body)
        .and_then(|_| file.write_all(b"\n"))
        .and_then(|_| file.sync_all())
        .map_err(|_| "delivery_dispatch_write_failed".to_owned())?;
    drop(file);
    fs::rename(&tmp, &path).map_err(|_| "delivery_dispatch_write_failed".to_owned())?;
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        fs::set_permissions(&path, fs::Permissions::from_mode(0o600))
...
pub fn load_delivery_dispatch(
    portable_data: &Path,
    dispatch_id: &str,
) -> Result<DeliveryDispatchRecord, String> {
    if !valid_dispatch_id(dispatch_id) {
        return Err("delivery_dispatch_dispatch_id_invalid".to_owned());
    }
    let path = delivery_dispatch_path(portable_data, dispatch_id);
    let metadata =
        fs::symlink_metadata(&path).map_err(|_| "delivery_dispatch_not_found".to_owned())?;
    if metadata.file_type().is_symlink() || !metadata.is_file() {
        return Err("delivery_dispatch_store_invalid".to_owned());
    }
    let raw = fs::read_to_string(&path).map_err(|_| "delivery_dispatch_not_found".to_owned())?;
    let record: DeliveryDispatchRecord =
        serde_json::from_str(&raw).map_err(|_| "delivery_dispatch_decode_failed".to_owned())?;
    if record.schema_version != DELIVERY_DISPATCH_SCHEMA_VERSION {
        return Err("delivery_dispatch_unsupported_generation".to_owned());
    }
    Ok(record)
}

pub fn list_delivery_dispatches(
    portable_data: &Path,
) -> Result<Vec<DeliveryDispatchRecord>, String> {
    let root = delivery_dispatch_root(portable_data);
    if !root.is_dir() {
        return Ok(Vec::new());
    }
    ...
    let entries =
        fs::read_dir(&root).map_err(|_| "delivery_dispatch_store_unavailable".to_owned())?;
    for entry in entries.flatten() {
        ...
        let raw = fs::read_to_string(&path)
            .map_err(|_| "delivery_dispatch_store_unavailable".to_owned())?;
        let record = serde_json::from_str::<DeliveryDispatchRecord>(&raw)
            .map_err(|_| "delivery_dispatch_decode_failed".to_owned())?;
        if record.schema_version != DELIVERY_DISPATCH_SCHEMA_VERSION
...
pub fn new_delivery_dispatch_id() -> String {
    let nanos = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map(|value| value.as_nanos())
        .unwrap_or(0);
    format!("delivery-dispatch-{nanos}")
}
...
mod tests {
    ...
    fn persist_and_load_round_trip() {
        ...
        let main_conversation_path = std::env::temp_dir()
            .join("licoup-delivery-dispatch-main.jsonl")
            .to_string_lossy()
            .into_owned();
        let record = DeliveryDispatchRecord::new_delivery(
            "delivery-dispatch-1",
            "delivery.dispatch",
            "PLAN-FIXTURE",
            1,
            ...
            1,
            SessionMode::Resume,
            Some("native:codex:manager".to_owned()),
            Some("native:claude:child".to_owned()),
            Some("workflow:worker".to_owned()),
            Some(main_conversation_path.clone()),
        );
        persist_delivery_dispatch(&dir, &record).unwrap();
        let loaded = load_delivery_dispatch(&dir, "delivery-dispatch-1").unwrap();
        assert_eq!(loaded.dispatch_id, "delivery-dispatch-1");
        assert_eq!(loaded.state, DeliveryDispatchState::Accepted);
        assert_eq!(loaded.session_mode, SessionMode::Resume);
        assert_eq!(
            loaded.main_conversation_path.as_deref(),
            Some(main_conversation_path.as_str())
        );
        assert_eq!(loaded.plan_code, "PLAN-FIXTURE");
        assert_eq!(loaded.role, "worker");
        let _ = fs::remove_dir_all(&dir);
    }

    #[test]
    fn delivery_dispatch_store_rejects_path_traversal_and_unknown_generations() {
        let dir = std::env::temp_dir().join(format!(
            "licoup-delivery-dispatch-invalid-{}",
            unix_ms_now()
        ));
        fs::create_dir_all(&dir).unwrap();
        let record = DeliveryDispatchRecord::new_delivery(
        ...
        assert_eq!(
            persist_delivery_dispatch(&dir, &record).unwrap_err(),
            "delivery_dispatch_dispatch_id_invalid"
        );
        let _ = fs::remove_dir_all(&dir);
    }
