...
use crate::domain::{
    ...
    delivery_state::{
        self, DeliveryDispatchRecord, DeliveryDispatchState, SessionMode, UsageSettlementState,
    },
};
    ...
    pub fn from_client_state() -> DeliveryResult<Self> {
        let state = crate::platform::client_state::state_get(
            crate::ffi::generated::client_state::ClientStateGetRequest {
                collection: crate::ffi::generated::client_state::ClientStateCollection::Settings,
            },
        )
        .map_err(|_| {
            DeliveryError::new(
    ...
    E: DeliveryExecutor,
{
    ...
    fn dispatch_one(
        ...
        let mut dispatch = DeliveryDispatchRecord::new_delivery(
            dispatch_id.clone(),
            "delivery.dispatch",
        ...
        dispatch.conversation_path = Some(child.source_path.clone());
        let portable = crate::platform::paths::portable_data_dir().map_err(|_| {
            DeliveryError::new(
                "delivery_dispatch_store_unavailable",
                "dispatch",
                "delivery-dispatch",
                true,
        ...
        delivery_state::persist_delivery_dispatch(&portable, &dispatch).map_err(|_| {
            DeliveryError::new(
                "delivery_dispatch_store_unavailable",
                "dispatch",
                "delivery-dispatch",
        ...
        match result.terminal {
            TerminalState::Pending => {
                dispatch.state = DeliveryDispatchState::Running;
                dispatch.usage_settlement = UsageSettlementState::Ready;
                report.pending += 1;
            }
            TerminalState::Completed | TerminalState::Failed | TerminalState::Cancelled => {
                self.finish(&mut dispatch, result, report)?;
            }
        }
        delivery_state::persist_delivery_dispatch(&portable, &dispatch).map_err(|_| {
            DeliveryError::new(
    ...
    fn finish(
        &mut self,
        dispatch: &mut DeliveryDispatchRecord,
        result: DispatchResult,
        report: &mut ScheduleReport,
    ) -> DeliveryResult<()> {
        let node_id = format!("{}:{}", self.workflow_id, dispatch.dispatch_id);
        ...
        dispatch.state = match result.terminal {
            TerminalState::Completed => DeliveryDispatchState::Completed,
            TerminalState::Failed => DeliveryDispatchState::Failed,
            TerminalState::Cancelled => DeliveryDispatchState::CancelRequested,
            TerminalState::Pending => DeliveryDispatchState::Running,
        };
        dispatch.usage_settlement = UsageSettlementState::Settled;
        dispatch.conversation_path = Some(result.conversation.source_path);
        if result.terminal == TerminalState::Failed {
            dispatch.error_code = Some("native_terminal_failed".to_owned());
        }
        match dispatch.role.as_str() {
            ...
            "worker" if result.terminal == TerminalState::Completed => {
                ...
                if running {
                    self.engine
                        .complete_dispatch(&dispatch.dispatch_id, Vec::new())
                        .map_err(DeliveryError::from)?;
                }
    ...
    fn reconcile_pending(&mut self, report: &mut ScheduleReport) -> DeliveryResult<()> {
        ...
        for mut dispatch in delivery_state::list_delivery_dispatches(&portable).map_err(|_| {
            DeliveryError::new(
                "delivery_dispatch_store_unavailable",
                "recovery",
                "delivery-dispatch",
            ...
            if dispatch.plan_code != self.engine.plan().code
                || !dispatch
                    .dispatch_id
                    .starts_with(&format!("{}:", self.workflow_id))
                || !matches!(
                    dispatch.state,
                    DeliveryDispatchState::Accepted | DeliveryDispatchState::Running
                )
            {
                continue;
            }
            let Some(path) = dispatch.conversation_path.clone() else {
                continue;
            };
            let conversation =
                self.executor
