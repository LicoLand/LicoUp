...
impl ContinuousNodeDriver {
    ...
    pub fn drive_step(&mut self) -> Result<DriverStepOutcome, DriverError> {
        ...
        if let Some(item) = self.queue.dequeue() {
            ...
            let payload: AdmittedMessagePayload = match serde_json::from_value(item.content.clone())
            {
                Ok(p) => p,
                Err(_) => {
                    // Item content was not an admitted message payload
                    return Ok(outcome);
                }
            };
                ...
                match status {
                                ...
                                ControlOperation::SubmitTask { input } => {
                                    if self.scope_barrier_active {
                                        outcome.reports.push(DispatchedEffectReport {
                                            delivery_id: recipient.delivery_id.clone(),
                                            recipient_node_id: recipient.node_id.clone(),
                                            status: RecipientEffectStatus::StateChanged {
                                                expected: recipient.admitted_state,
                                                current: facade.lifecycle_state,
                                            },
                    ...
                    RecipientEffectStatus::GenerationStale { expected, current } => {
                        ...
                        outcome.reports.push(DispatchedEffectReport {
                            delivery_id: recipient.delivery_id,
                            recipient_node_id: recipient.node_id,
                            status: RecipientEffectStatus::GenerationStale { expected, current },
                            operation: op_str.clone(),
                            invocation_id: None,
                        });
                    }
                ...
                if let Ok(status) = facade.adapter.poll_status(inv_id) {
                    match status {
                        AdapterExecutionStatus::Completed { output } => {
                            settled_reentries.push(ResultReentry {
                                source_principal_id: "adapter".to_string(),
                                graph_id: self.graph_id.clone(),
                                node_id: node_id.clone(),
                                invocation_id: inv_id.clone(),
                                generation: facade.generation,
                                cursor: self.durable_cursor.saturating_add(1),
                                attempt_token: facade
                                    .active_invocation
                                    .as_ref()
                                    .map(|i| i.attempt_token.clone())
                                    .unwrap_or_default(),
                                outcome: NodeExecutionOutcome::Success { output },
                            });
                        }
                        AdapterExecutionStatus::Failed { error, retryable } => {
                            settled_reentries.push(ResultReentry {
                                ...
                                attempt_token: facade
                                    ...
                                    .map(|i| i.attempt_token.clone())
                                    .unwrap_or_default(),
                                outcome: NodeExecutionOutcome::Failure { error, retryable },
                            });
                        }
                        AdapterExecutionStatus::Cancelled { acknowledged } => {
                            settled_reentries.push(ResultReentry {
                                ...
                                attempt_token: facade
                                    ...
                                    .map(|i| i.attempt_token.clone())
                                    .unwrap_or_default(),
                                outcome: NodeExecutionOutcome::Cancelled { acknowledged },
                            });
                        }
                        AdapterExecutionStatus::Running | AdapterExecutionStatus::Suspended => {}
                    }
                }
            }
        }
...
mod tests {
    ...
    fn test_driver_result_reentry_with_source_identity() {
        ...
        let receipt = driver
            .reenter_result(ResultReentry {
                source_principal_id: "worker-backend".into(),
                graph_id: "graph-1".into(),
    ...
    fn test_driver_independent_node_concurrency() {
        ...
        driver
            .reenter_result(ResultReentry {
                source_principal_id: "backend".into(),
    ...
    fn test_driver_subscription_auto_resume_on_matching_event() {
        ...
        let receipt = driver
            .reenter_result(ResultReentry {
                source_principal_id: "external".into(),
                graph_id: "graph-sub".into(),
                node_id: "node-paused".into(),
