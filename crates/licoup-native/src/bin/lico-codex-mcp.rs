use licoup_native::platform::orchestrator_control_plane::{
    build_codex_mcp_orchestrator_request, build_codex_mcp_status_event_request,
};
use licoup_native::platform::orchestrator_ipc::{OrchestratorIpcClient, OrchestratorIpcReceipt};
use licoup_native::platform::orchestrator_service::default_orchestrator_state_root;
...
const SERVER_NAME: &str = "lico-up-orchestration";
...
fn execute_status(
    ...
    cancelled: Arc<AtomicBool>,
    timeout: Duration,
    first_request: licoup_native::platform::orchestrator_ipc::OrchestratorIpcRequest,
    root: PathBuf,
) -> Value {
...
fn project_receipt(name: &str, source: &Value, events: Option<&Value>) -> Value {
    ...
    result.insert("schemaVersion".into(), json!("licoup.mcp.receipt.v1"));
...
fn output_schema() -> Value {
    object_schema(
        json!({
            "schemaVersion": {"type": "string", "const": "licoup.mcp.receipt.v1"},
