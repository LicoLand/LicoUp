// Hermes persistent-ACP semantic state machine owned by its parser adapter.
...
pub(in crate::platform) struct ParsedProtocolFrame {
...
impl SessionProtocol {
    ...
    pub(in crate::platform) fn handle_message(&mut self, message: Value) -> Vec<ProtocolEffect> {
        ...
        if crate::platform::native_agent_parser::adapters::hermes::is_notification(&message) {
            return self.handle_notification(message);
        }
        match self.phase {
            ProtocolPhase::AwaitInitialize
                if crate::platform::native_agent_parser::adapters::hermes::response_id_matches(
                    &message,
                    INITIALIZE_REQUEST_ID,
                ) =>
            {
    ...
    pub(in crate::platform) fn handle_frame(&mut self, line: &[u8]) -> ParsedProtocolFrame {
        let message =
    ...
    pub(in crate::platform) fn handle_server_request(
        &mut self,
        request: crate::platform::native_agent_parser::adapters::hermes::PermissionRequest,
    ...
    pub(in crate::platform) fn handle_prompt_response(
        ...
        let stop_reason =
            match crate::platform::native_agent_parser::adapters::hermes::prompt_stop_reason(
                message,
                PROMPT_REQUEST_ID,
            ) {
    ...
    pub(in crate::platform) fn handle_notification(
        ...
        let update = match crate::platform::native_agent_parser::adapters::hermes::session_update(
            &message,
