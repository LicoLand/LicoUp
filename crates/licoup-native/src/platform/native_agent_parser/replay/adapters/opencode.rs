//! Replay arm for the `opencode` adapter.
//!
...
use crate::platform::native_agent_parser::adapters::opencode::{
...
impl FrameReplay for Replay {
    fn feed(&mut self, frame: &RecordedFrame) -> Result<Vec<Value>, String> {
        ...
        {
            // `opencode_driver::open_session` binds the stream parser to the
