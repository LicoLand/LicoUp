...
impl OrchestratorServiceLifecycle {
    pub fn discover_or_start(
        ...
        if executable.file_stem().and_then(|value| value.to_str()) != Some("licoup") {
            executable.set_file_name(format!("licoup{}", std::env::consts::EXE_SUFFIX));
        }
        ...
        std::process::Command::new(executable)
            ...
            .env_clear()
            .env("LICOUP_STATE_ROOT", state_root)
...
pub fn default_orchestrator_state_root() -> Result<PathBuf> {
    if let Some(value) = std::env::var_os("LICOUP_STATE_ROOT") {
