...
#[derive(Debug)]
pub(in crate::platform) struct ParkedPermission {
    #[allow(dead_code)]
    request_id: Value,
    session_id: String,
    turn_id: String,
...
pub(in crate::platform) fn parked_permissions() -> &'static Mutex<HashMap<String, ParkedPermission>>
{
...
pub fn resolve_parked_permission(token: &str, allow: bool) -> Result<Value, &'static str> {
    ...
    if let Some(parked) = parked {
        ...
        return Ok(json!({
            "ok": true,
            "agentId": parked.requester_agent_id,
    ...
    }
    {
        ...
        if guard.len() >= MAX_PARKED_PERMISSIONS {
            return Err(ProtocolFailure::new(
                "approval_park_capacity",
                "Approval park capacity was exceeded.",
                "server/request",
            ));
        }
        guard.insert(
    ...
    fn insert_test_park(token: &str, created_at: Instant) -> mpsc::Receiver<bool> {
        ...
        parked_permissions().lock().unwrap().insert(
            ...
            ParkedPermission {
                ...
                requester_agent_id: "hermes".to_string(),
