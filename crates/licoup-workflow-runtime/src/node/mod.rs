...
//!
//! * A node visit has at most one effect in flight. A second claim of a visit
//!   that is still running is refused instead of quietly tracked twice, so
//!   "commit A's outcome, then admit A's successors" cannot decay into
//!   "start a successor of a visit that has not settled".
...
/// The effects one owner has in flight, keyed by node visit.
#[derive(Debug, Default)]
pub struct NodeLedger {
    live: BTreeMap<NodeVisitKey, NodeEffect>,
    by_command: BTreeMap<String, NodeVisitKey>,
    settled: usize,
}
...
impl NodeLedger {
    ...
    pub fn record_claim(
        ...
        let key = NodeVisitKey::from_command(command);
        ...
        if let Some(existing) = self.live.get(&key) {
            return Err(NodeLifecycleError::VisitAlreadyInFlight {
                key,
                command_id: existing.command_id.clone(),
            });
        }
        self.by_command.insert(command.id.clone(), key.clone());
        self.live.insert(
            key.clone(),
            NodeEffect {
                key: key.clone(),
    ...
    pub fn freeze(&self, target: &ControlTarget) -> FrozenRecipients {
        ...
        for effect in self.live.values() {
            let addressed = match target {
                ControlTarget::Run => true,
                ControlTarget::Node(key) => effect.key == *key,
            };
            if addressed {
                frozen.commands.push(effect.command_id.clone());
                frozen.visits.push(effect.key.clone());
            }
        }
        frozen
    }
    ...
    /// The commands that are in flight, in visit order.
    ...
    fn key_of(&self, command_id: &str) -> Result<&NodeVisitKey, NodeLifecycleError> {
        self.by_command
            .get(command_id)
            .ok_or_else(|| NodeLifecycleError::UnknownCommand {
                command_id: command_id.to_owned(),
