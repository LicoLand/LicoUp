...
export function inferHostTargetId(catalog, authorityIds) {
  ...
  const match = catalog.targets.find((target) =>
    ...
    target.packageBuildSupported === true && authorityIds.includes(target.id));
  requireValue(match,
    `no package-acceptance client target matches the current host (${platform}/${arch})`);
  return match.id;
}
...
export function selectedTargetIds(catalog, authorityIds) {
  ...
  const requested = explicit
    ...
    : [inferHostTargetId(catalog, authorityIds)];
