...
const releaseTargetsCatalogPath = path.join(
  repoRoot,
  "tools",
  "client-release-targets.json",
);

// Installer strategies that make a target eligible for in-place client
// updates; store/registry channels stay out of the update manifest.
const DESKTOP_INSTALLER_STRATEGIES = new Set([
  "app-bundle-replacement",
  "msix-or-portable-replacement",
  "appimage-deb-rpm-or-tar",
]);
...
function selectedTargetIds(value) {
  return String(value || "")
    ...
    .map((entry) => {
      const [target, selected] = entry.split("=");
      if (!CLIENT_RELEASE_TARGETS[target] || !["true", "false"].includes(selected)) {
        fail("invalid target selection");
      }
      return { target, selected: selected === "true" };
    });
}
...
function buildManifest(args) {
  ...
  const releaseTargets = JSON.parse(readFileSync(releaseTargetsCatalogPath, "utf8"));
  const catalog = new Map(
    (releaseTargets.targets || []).map((target) => [target.id, target]),
  );

  const selected = selectedTargetIds(args.targets).filter((entry) => entry.selected);
  const artifacts = [];
  for (const { target } of selected) {
    const meta = catalog.get(target);
    if (!meta) fail(`release target catalog is missing target ${target}`);
    if (!DESKTOP_INSTALLER_STRATEGIES.has(meta.installerStrategy)) continue;
    ...
    if (!artifactName || !localNames.has(artifactName)) {
      fail(`selected target ${target} artifact is missing from the asset set`);
    }
    const filePath = path.join(assetsRoot, artifactName);
    const info = statSync(filePath);
    if (!info.isFile() || info.size <= 0) fail(`selected target ${target} artifact is invalid`);
    const artifact = {
      targetId: target,
      platform: meta.platform,
      osFamily: meta.osFamily,
      arch: meta.arch,
      installerStrategy: meta.installerStrategy,
      url: `https://github.com/${repository}/releases/download/${tag}/${artifactName}`,
      fileName: artifactName,
      size: info.size,
      sha256: `sha256:${sha256Hex(readFileSync(filePath))}`,
    };
    if (meta.installerStrategy === "app-bundle-replacement") {
