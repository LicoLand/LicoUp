...
export class StdioRpcClient {
  ...
  #finalizeStreaming(requestId) {
    ...
    try {
      ...
      const completedMatchesTerminal = completedOutput === result.output;
      // Some lanes emit non-final chunk aggregates (e.g. thinking) while the
      // completed/terminal texts still agree. Accept that shape when streaming
      // chunks were observed and the completed text matches the terminal.
      requireFact(
        (chunkMatchesCompleted && chunkMatchesTerminal)
          || (pending.chunks.length > 0 && completedMatchesTerminal && result.output.length > 0),
