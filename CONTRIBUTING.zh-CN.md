...
真实响应或传输检查要花真 token，所以使用够用的最便宜模型。每个 Agent 的
选型只有一个文件：
...
要改测试模型只改这里；不得通过继承的环境变量选用更贵的模型或思考档位。响应检查使用
最低支持的思考档位，只发送一次 `Hi`，接受 Agent 原始回复，不要求指定词语、数字、
标记或输出格式。只有验证同一会话续接时才增加短回合。
禁止通过倒数、重复、长列表或无限输出维持付费回合。长时间流式输出、等待与取消竞争
使用可控的本地夹具验证。真实控制检查可发送短问候；若控制前回复已经结束，该项保持
未验证，不以此为由加长输出、重复付费探测或升级模型。连通性检查应保持其原定目的。
本地夹具只证明长流、取消和事件时序等技术行为，不能宣称长任务能力已经通过。长任务端到端
验收使用既有授权范围内的真实 Issue。执行 Agent 利用可用的探测与评估，自主选择合适且
成本合理的模型，并根据实际表现调整，不为选模反复请求批准。响应检查的模型配置不约束
真实 Issue 工作；优先复用已有评估结果，避免没有任务价值的额外探测。
...
- 真实响应或传输检查使用验证模型权威中够用的最便宜模型；真实 Issue 工作由执行 Agent
  根据任务自主选模。
